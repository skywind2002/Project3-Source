...
> 补充 By 于子涵：随着 escape_prob_threshold 的减小，逃逸码覆盖的码字将会减少、码本中的码字将会增多、编码后的码流将会缩短，但是与此同时码本本身也会增长。所以并不需要将计算效率纳入考虑来得到 “平衡”，只要将码本本身的长度也考虑进去就可以自然得到 “平衡” 了。
...
修改上面函数中的`symbol_count`为2，重新进行实验，得到结果：
...
门限为0的时候总比特数高是因为码本有约82kbits。之后逐渐减小到200kbits是因为码本变短；再变小是因为相邻像素双符号编码起的作用。之后再升高则是因为为使用任何的优化方式。可以看到实际选择`0.65-0.85`能达到算力与效率的大致平衡。