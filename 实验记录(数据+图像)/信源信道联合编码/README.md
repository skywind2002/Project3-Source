...
不使用卷积码、并使用 2ASK 电平映射方式，选择 H.261 量化方式、双符号编码、逃逸码总概率占比 $0.9$、条带高度取 $4$，在量化参数 `quant_factors=[5,10,50,100]` 的条件下分别进行试验。每组试验取 10 次重复试验的平均值和一倍标准差。得到基准曲线

![](./基准曲线.png)

使用 $1/2, 1/3$ 效率卷积码，重复以上操作（因为 Viterbi 速度较慢，这时使用 5 次重复试验的均值和一倍标准差）得到结果

![](./12卷积码.png)
![](./13卷积码.png)

可见 PSNR 明显提升，传输效果明显变好。